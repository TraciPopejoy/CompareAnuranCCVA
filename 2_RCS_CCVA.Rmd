...
#     print(head(dat_sp))
    ...
    CS_out<-bind_rows(CS_out, CS_out_v)
      ...
      print(paste(nrow(dat_sp), 'in the study region'))
       area.i=0
       used.nrow<-0; min.year = NA; area.sqkm = 0; all.poly.nrow<-all.poly.n<-0
...
```{r calculate the RCS at many scales, warning=F}
RCS_comp_all<-NULL
start.time<-Sys.time()
for(spp in anuran.taxa){
  buff_out<-NULL
  for(buf.ext in c("spat.ext", "huc3","huc6", "huc8","al","fl","sc","nc","ga","ms")){
...
write.csv(RCS_comp_all, 'b_RegionalRCS/buffersmainly.csv', row.names = F)
```


```{r code to show where climate NAs can occur, echo=F, warning=F}
...
for(i in list.files(PATH_out)){
  all.spp.huc<-bind_rows(all.spp.huc,
                         read.csv(paste0('b_RegionalRCS/spp_hucs/', i), colClasses = c('character','integer','numeric')) %>%
                           mutate(scientific_name=gsub('_.*','',i),
                                  climate_var=gsub('_','',substr(i, nchar(i)-7, nchar(i)-4))))
}

# join with huc12 tibble 
main.huc.df<-left_join(all.spp.huc %>% pivot_wider(names_from=climate_var, values_from=clim_sum_val),
          huc12s %>% as_tibble() %>% select(huc2, huc12, WSAREA, states),
          by='huc12')
huc.states<-NULL
for(stt in c('NC', "AL","FL","SC","GA","MS")){
  st.huc<-main.huc.df %>% 
    filter(grepl(stt, states)) 
  area.i<-st.huc %>% distinct(scientific_name, huc12, .keep_all=T) %>%
    group_by(scientific_name) %>%
    dplyr::summarize(area.type=paste0(tolower(stt), '_WS'),
                     area.sqkm=sum(WSAREA))
  huc.sum<-st.huc %>% distinct(scientific_name, huc12, year, .keep_all=T) %>%
    group_by(scientific_name) %>%
...
write.csv(RCS_comp_all, 'b_RegionalRCS/regional_rcs_011023.csv', row.names = F)
```
...
rcs.plot.df<-rcs_res %>%
  mutate(spF=factor(scientific_name, levels=rcs_ord),
         ext.type=case_when(grepl('spat.ext', area.type)|grepl('huc12', area.type)~'entire extent',
                            substr(area.type,1,2) %in% c('al','nc','sc','ga','ms','fl')~ 'state',
                            substr(area.type, 1,3) =='huc'~'HUC2'),
         ext.subtype=case_when(grepl('spat.ext', area.type)|grepl('huc12', area.type)~'entire extent',
                            substr(area.type,1,2) %in% c('al','nc','sc','ga','ms','fl')~ substr(area.type, 1,2),
                            substr(area.type, 1,3) =='huc'~gsub('_','',gsub('0','',substr(area.type, 1,5)))),
