...
Below are potential subregions to run the analysis at. Meryl expressed some hesitency about this because of the restrictions placed out our species pool. It is difficult to interpret results when not all species are include or species lists differ g
...
We then apply this function at the different grain sizes and spatial extents to create a single data frame with the RCS components. 

```{r calculate the RCS at many scales, warning=F, eval=T}
RCS_comp_all<-NULL
start.time<-Sys.time()
...
We want to include L. sevosus even though it does not have any occurrence points in our dataset. In stead, we will use the IUCN range as our estimate of AOO. This range was compiled in 2008 by information from S. Richter, unpublished data. As such, we will use 2008 as our value for the climate moving window analysis.
...
Rarely, part of the AOO returns climate values as NA. This generally occurs near the coast and represents that part of the point buffer might be outside the PRISM raster. The plot below demonstrates this for *Acris gryllus*.

```{r code to show where climate NAs can occur, echo=F, warning=F, eval=T}
...
By writing out the individual occupied HUC information for each species, I can use the states column native to the USGS Watershed Boundary Dataset or the huc ID string to subset the huc12s based on our subregion boundaries. The code below does that.

```{r evaluate huc3, eval=T}
all.spp.huc<-NULL
...
Now, this way of subsetting hucs without recalculating the RCS means that our subregional state RCS components calculated at the watershed grain size do incorporate some area outside of the state. Below is an example for Alabama. The added area is small.

```{r extent pic, fig.width=4, fig.height=2.5, eval=T}
ggplot()+
...
Finally, we output the results to analyze in 5_Compare_CCVAs.Rmd!
